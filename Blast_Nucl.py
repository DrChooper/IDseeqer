...
def Search_Blast(taxaid) : 
	...
	q0 = select([settings.blast_input.c.seq_nucl,settings.blast_input.c.id])
	q2 = q0.where(settings.blast_input.c.locus_seq==None)
	q3 = q2.where(settings.blast_input.c.seq_nucl!=None)
	...
	os.system("blastn -max_hsps 1 -max_target_seqs 5 -outfmt 6 -query query_nucl_"+taxaid+".fasta -db blastdb_"+taxaid+"_nucl -out nucl_"+taxaid+".tsv") # -task blastn-short")
	...
	df = pd.read_csv("nucl_"+taxaid+".tsv",header = 0, sep = "\t", names=header)#ENTER THE taxa here

	try :
		os.remove("query_nucl_" + taxaid + ".fasta")
		os.remove("nucl_" + taxaid + ".tsv")
