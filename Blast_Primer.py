...
def updatedb_primer(recs, taxaid):
...
def Search_Blast(taxaid) : 
	...
	q0 = select([settings.blast_input.c.seq_primer,settings.blast_input.c.id])
	q2 = q0.where(settings.blast_input.c.locus_primer==None)
	q3 = q2.where(settings.blast_input.c.seq_primer!=None)
	q1 = q3.where(settings.blast_input.c.taxa==int(taxaid)) #ENTER TAXA FOR PLANT HERE
	qn = q1.limit(settings.blast_chunk)
	rec = list(settings.engine.execute(qn).fetchall())
	
	taxaid = str(taxaid).strip()

	records = [SeqRecord(id=str(r.id),seq=Seq(r.seq_primer))for r in rec] 

	SeqIO.write(records,"query_nucl_"+taxaid+".fasta","fasta")# ENTER THE species taxa here and in the blast query below (twice)!!
	#Enter the right blast database
	os.system("blastn -max_hsps 1 -max_target_seqs 5 -outfmt 6 -query query_nucl_"+taxaid+".fasta -db blastdb_"+taxaid+"_nucl -out primer_"+taxaid+".tsv -task blastn-short")
	...
	df = pd.read_csv("primer_"+taxaid+".tsv",header = 0, sep = "\t", names=header)#ENTER THE taxa here

	# Remove temporary files
	try :
		os.remove("query_nucl_" + taxaid + ".fasta")
		os.remove("primer_" + taxaid + ".tsv")
